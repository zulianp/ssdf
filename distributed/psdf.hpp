...
namespace ssdf {
    ...
    static void compute_aabb(MPI_Comm comm,
...
#pragma omp parallel for reduction(min : xmin_v, ymin_v, zmin_v) reduction(max : xmax_v, ymax_v, zmax_v)
        ...
        T aabb_minmax[6] = {xmin_v, ymin_v, zmin_v, -xmax_v, -ymax_v, -zmax_v};
        MPI_Allreduce(MPI_IN_PLACE, aabb_minmax, 6, mpi_type<T>(), MPI_MIN, comm);
    ...
    int pedf_update(MPI_Comm comm,
        ...
        G aabb_minmax[6] = {std::numeric_limits<G>::max(),
                            std::numeric_limits<G>::max(),
                            std::numeric_limits<G>::max(),
                            std::numeric_limits<G>::lowest(),
                            std::numeric_limits<G>::lowest(),
                            std::numeric_limits<G>::lowest()};
#pragma omp parallel
        {
...
#pragma omp critical
            {
                aabb_minmax[0] = std::min(aabb_minmax[0], tminx);
                aabb_minmax[1] = std::min(aabb_minmax[1], tminy);
                aabb_minmax[2] = std::min(aabb_minmax[2], tminz);
                aabb_minmax[3] = std::max(aabb_minmax[3], tmaxx);
                aabb_minmax[4] = std::max(aabb_minmax[4], tmaxy);
                aabb_minmax[5] = std::max(aabb_minmax[5], tmaxz);
            }
        }
        ...
        MPI_Allgather(&aabb_minmax[0], 1, mpi_type_G, all_pminx.data(), 1, mpi_type_G, comm);
        MPI_Allgather(&aabb_minmax[3], 1, mpi_type_G, all_pmaxx.data(), 1, mpi_type_G, comm);
        MPI_Allgather(&aabb_minmax[1], 1, mpi_type_G, all_pminy.data(), 1, mpi_type_G, comm);
        MPI_Allgather(&aabb_minmax[4], 1, mpi_type_G, all_pmaxy.data(), 1, mpi_type_G, comm);
        MPI_Allgather(&aabb_minmax[2], 1, mpi_type_G, all_pminz.data(), 1, mpi_type_G, comm);
        MPI_Allgather(&aabb_minmax[5], 1, mpi_type_G, all_pmaxz.data(), 1, mpi_type_G, comm);

        auto intersects = [](G minx1,
                             G maxx1,
                             G miny1,
                             G maxy1,
                             G minz1,
...
#pragma omp parallel for
        ...
        MPI_Alltoallv(send_vbuf_x.data(),
                      send_vcounts.data(),
                      send_vdisp.data(),
                      mpi_type_G,
                      recv_sx.data(),
                      ...
                      mpi_type_G,
                      comm);
        MPI_Alltoallv(send_vbuf_y.data(),
                      send_vcounts.data(),
                      send_vdisp.data(),
                      mpi_type_G,
                      recv_sy.data(),
                      ...
                      mpi_type_G,
                      comm);
        MPI_Alltoallv(send_vbuf_z.data(),
                      send_vcounts.data(),
                      send_vdisp.data(),
                      mpi_type_G,
                      recv_sz.data(),
        ...
        PSDF_TIMER(comm, edf_select);
        return ssdf::edf_select<G, T, I>(lnpoints,
                                         x,
                                         y,
                                         z,
